...
export async function buildLiveSnapshot(asOf = new Date().toISOString()): Promise<DeskSnapshot> {
  ...
  return {
    ...
    predict,
    sourceNote: live
      ? `Odds API board (${listed.join(", ")}) · Updated ${new Date(asOf).toLocaleString("en-US", { timeZone: "America/New_York", month: "short", day: "numeric", hour: "numeric", minute: "2-digit", hour12: true })} ET.${publicSplits.length ? ` Ticket/handle tape on ${publicSplits.length} game${publicSplits.length === 1 ? "" : "s"}.` : ""} Lines are research, not a lock.`
...
function overlayOddsApiMains(quotes: QuoteLine[], oddsApiData: any[]): QuoteLine[] {
  ...
  for (const q of quotes) {
    ...
    if (matchingGame && matchingGame.bookmakers) {
      ...
      if (q.marketType === 'ml' && ml && ml.outcomes) {
      ...
      if (q.marketType === 'spread' && sp && sp.outcomes) {
      ...
      if (q.marketType === 'total' && tot && tot.outcomes) {
