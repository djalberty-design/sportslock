 ...
 * 60/40 with season — ¢â‚¬â„¢— — — ¢â‚¬â„¢— ¢â‚¬â„¢— — ¢â‚¬â„¢— ¢â‚¬â„¢— ¢â‚¬â„¢— — ¢â‚¬â„¢— ¢â‚¬â„¢— not a hit-rate sticker.
...
export function parsePropSelection(
  ...
  if (!player) {
    ...
    player = cut.replace(/[-—_]/g, " ").replace(/\s+/g, " ").trim();
  }
...
function rateOver(stat: PropStat, rate: number, line: number): number {
  // Yards / points / PRA are continuous — ¢â‚¬â„¢— — — ¢â‚¬â„¢— ¢â‚¬â„¢— — ¢â‚¬â„¢— ¢â‚¬â„¢— ¢â‚¬â„¢— — ¢â‚¬â„¢— ¢â‚¬â„¢— Poisson understates the tails (DMP Learn 2026).
...
export function buildPropChance(input: PropInput): PropReport {
  ...
  push(layers, {
    ...
    precision: 14,
    family: "market",
    note: "The live Hard Rock Bet Florida number, with the extra juice player bets usually carry stripped off. This is the prior — ¢â‚¬â„¢— — — ¢â‚¬â„¢— ¢â‚¬â„¢— — ¢â‚¬â„¢— ¢â‚¬â„¢— ¢â‚¬â„¢— — ¢â‚¬â„¢— ¢â‚¬â„¢— liquid prop markets are hard to beat.",
  });
  ...
  if (parsed.counting && parsed.line != null && Number.isFinite(parsed.line)) {
    ...
    if (rate != null && rate > 0) {
      ...
      push(layers, {
        ...
        family: "game",
        note: usedRecent
          ? `${parsed.player} recency-weighted last ${input.recentN ?? 10}: ${input.recentRate!.toFixed(2)} ${parsed.statLabel} (60%) blended with season ${input.seasonRate != null ? input.seasonRate.toFixed(2) : "n/a"} (40%). Last-10 of THIS stat, not a W-L sticker — ¢â‚¬â„¢— — — ¢â‚¬â„¢— ¢â‚¬â„¢— — ¢â‚¬â„¢— ¢â‚¬â„¢— ¢â‚¬â„¢— — ¢â‚¬â„¢— ¢â‚¬â„¢— books already shade hot streaks.`
    ...
    pushEmpty(
      ...
      "last10",
      "Last 10 of this stat",
      `Looked up last-10 of ${parsed.statLabel}. Yes/no tickets do not take a counting line — ¢â‚¬â„¢— — — ¢â‚¬â„¢— ¢â‚¬â„¢— — ¢â‚¬â„¢— ¢â‚¬â„¢— ¢â‚¬â„¢— — ¢â‚¬â„¢— ¢â‚¬â„¢— empty counting look, not a skip.`,
      "game",
    );
  }
  ...
  if (input.oppAllowed != null && Number.isFinite(input.oppAllowed) && input.oppAllowed > 0) {
    ...
    push(layers, {
      ...
      note: hitting
        ...
        : `Opponent environment ${input.oppAllowed.toFixed(1)} vs typical ${leagueOpp}. Last-10 allowed / live team stats — ¢â‚¬â„¢— — — ¢â‚¬â„¢— ¢â‚¬â„¢— — ¢â‚¬â„¢— ¢â‚¬â„¢— ¢â‚¬â„¢— — ¢â‚¬â„¢— ¢â‚¬â„¢— not a made-up rank.`,
    });
  } else {
    pushEmpty(
      layers,
  ...
  if (input.pitcherEra != null && Number.isFinite(input.pitcherEra) && input.pitcherEra > 0) {
    ...
    push(layers, {
      ...
      family: "game",
      note: hitting
        ? `Opposing starter ERA ${input.pitcherEra.toFixed(2)}. Live probable from ESPN — ¢â‚¬â„¢— — — ¢â‚¬â„¢— ¢â‚¬â„¢— — ¢â‚¬â„¢— ¢â‚¬â„¢— ¢â‚¬â„¢— — ¢â‚¬â„¢— ¢â‚¬â„¢— the pitcher-vs-batter number we can actually fetch tonight.`
  ...
  if (input.ownUnderlying != null && Number.isFinite(input.ownUnderlying)) {
    ...
    push(layers, {
      ...
      precision: 1.8,
      family: "game",
      note: `Season ${input.seasonRate.toFixed(2)} vs last-10 ${input.recentRate.toFixed(2)} ${parsed.statLabel}. Process vs a hot/cold streak — ¢â‚¬â„¢— — — ¢â‚¬â„¢— ¢â‚¬â„¢— — ¢â‚¬â„¢— ¢â‚¬â„¢— ¢â‚¬â„¢— — ¢â‚¬â„¢— ¢â‚¬â„¢— we do not treat 8/10 as a lock.`,
      thin: true,
    });
  } else {
    pushEmpty(
  ...
  const customize = photographedIsOver
    ? hit >= 0.52
      ? `The model agrees with the photographed over. Still confirm the live Hard Rock number — delayed print is not a fill.`
