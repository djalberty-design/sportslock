/**
 * AI Picks desk â€” rank every ticket the book posts, not just game winners.
 *
 ...
 * chance Ã— payout Ã— market-quality Ã— edge vs the juice.
...
/** Unique market cell (no bucket) â€” used to de-dupe sheet vs posted rows. */
 ...
 * NFL/CFB spreads get a key-number tilt (3 and 7) â€” Dimers / CLV desks treat those as physics.
 */
...
/**
 * Ranking score. deskScore (chanceÂ² Ã— âˆšpayout, tape nudge) is the core.
...
/** Quality badge is one scale. High â‰¥0.72, Med 0.55â€“0.71, Low <0.55. Chance does not rewrite the label. */
...
function stampDisplayedChance(reason: string, shown: number): string {
  ...
  const next = reason
    .replace(/Combined chance[^.]*â‰ˆ\s*[\d.]+(?:\s*in 100)?%?\.?/gi, `Combined chance â‰ˆ ${pct}.`)
    .replace(/about [\d.]+ in 100 tickets/gi, `about ${pct.replace("%", "")} in 100 tickets`);
  return next;
}

/** Displayed combined % from builder legs â€” same fromParlay calibration the catalog and /ticket use. */
...
function popularWhy(r: ScanRow): string {
  ...
  if (r.inPlay) {
    ...
    return `Live ticket. The public number is delayed â€” photograph Hard Rock now. Last-10 scores and the full ensemble still run; quality is haircut because a delayed live fill is not a prior. Not The Call.${tape}`;
  }
  const m =
    r.marketType === "ml"
      ? "Who wins the game â€” moneyline. Highest-info market on the board."
      : r.marketType === "spread"
...
function propWhy(r: ScanRow): string {
  return `${r.player ?? "This player"} â€” ${shortPick(r.selection, r.marketType)}. Last-10 games (when we have them) blended with season rate, today's total, script, park, weather, and who is listed out. College player bets are blocked in Florida. Photograph the live Hard Rock number â€” analysis, not a fill.`;
}
...
/** The Call must be a high-info ticket that still pays â€” not a noisy 0.5 inning. */
function heroEligible(p: DeskPick): boolean {
  if (p.parlay) return false;
  // Use isLiveDeskPick â€” checks inPlay flag, in_play tag, AND start time so
...
function fallbackParlay(rows: ScanRow[], reason: string): ParlayCandidate {
  ...
  return {
    ...
    reason: `${reason} Combined chance â‰ˆ ${formatChancePct(calibratedChance(combinedFair, decimalPayout > 1 ? 1 / decimalPayout : undefined, parlayInfoQuality(rows.length, sameGame))) ?? "â€”"}. Combined is closed form. Quality capped at 0.64. No fake sim %.`,
...
export function deskPickFromLegRefs(
  ...
  if (live) {
    pick.why = `${pick.why} Live leg on the slip â€” combined quality haircut. Never The Call. Never the gold ribbon.`;
  }
  if (stood.length) {
...
export function correlationFlag(corr?: ParlayCandidate["correlation"], sameGame?: boolean): string {
  if (corr === "shared-latent") return "joint-path Â· shared-latent";
...
export function ribbonSitWhy(p: DeskPick, rows?: ScanRow[]): string {
  ...
  if (live) {
    return "Not on the gold ribbon â€” live legs cannot load onto the ribbon. Combined quality is haircut. Never The Call.";
  }
  ...
  if (parlay.legs.some((l) => l.marketType === "prop")) {
    return "Not on the gold ribbon â€” player props are custom builder, never the gold badge.";
  }
  if (parlay.sameGame) {
    return "Not on the gold ribbon â€” same-game lives as SGP catalog, not the gold badge. Joint-path or Thin fallback-haircut still prints on the ticket.";
  }
  if (parlay.legs.some((l) => /1st inning|first inning/i.test(l.selection) && /0\.5/.test(l.selection))) {
    return "Not on the gold ribbon â€” a period 0.5 cannot be a ribbon leg.";
  }
  if (!ribbonEligible(p)) {
    return "Not on the gold ribbon â€” a leg missed the 56%/60% displayed floor, the combined floor, payout 1.45, or quality 0.72.";
  }
...
export function pickMatchup(p: DeskPick): string {
  ...
  if (p.parlay) {
    return p.parlay.legs.map((l) => `${l.away} at ${l.home}`).join(" Â· ");
  }
...
/** Same board in â†’ same string out. No clock, no random. */
