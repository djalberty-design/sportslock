...
export const authConfigured =
  !authDisabled && (Boolean(grokClientId && grokClientSecret) || Boolean(process.env.GOOGLE_CLIENT_ID));
...
export const SESSION_TOKEN_COOKIE = "sportslock.session_token";

const grokOAuthPlugin = authConfigured
  ? genericOAuth({
      config: GROK_PROVIDERS.filter(p => p.providerId !== 'google').map(({ providerId, idp }) => ({
        providerId,
...
export const auth = betterAuth({
  ...
  // Simple cookie names — no __Host- prefix (which requires no Domain attribute,
  // and Better Auth may add Domain derived from baseURL, causing browsers to
  ...
  advanced: {
    ...
    cookies: {
      ...
      session_data: { name: "sportslock.session_data" },
      account_data: { name: "sportslock.account_data" },
      dont_remember: { name: "sportslock.dont_remember" },
    },
  },
